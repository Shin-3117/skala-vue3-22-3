...
- 'ㅅㅇ'(서울), 'ㅇㅅ'(울산) 등 초성만 입력해도 원하는 도시를 즉시 필터링하여 검색할 수 있습니다.

### 5. 🌡️ 온도 단위 전환 (℃ / ℉) 및 다크 모드 (Pinia Store)
- `configStore`를 통한 전역 상태 관리로 대시보드 및 상세 페이지 전체의 온도 단위(섭씨/화씨)와 테마(라이트/다크)를 원클릭으로 전환할 수 있습니다.

### 6. 📱 동적 상세 페이지 (Vue Router Programmatic Navigation)
- 도시별 상세보기 클릭 시 `/weather/:cityId` 동적 라우트로 이동하여 날씨 맞춤형 복장/우산 가이드를 제공합니다.

### 7. 🎨 외부 UI 라이브러리 적용 (`shadcn-vue`)
- `shadcn-vue` 컴포넌트 시스템 구축: `src/components/ui/` 폴더 내 `Card`, `Button`, `Badge`, `ScrollArea`, `Input`, `Label` 등 컴포넌트를 구성하여 사용합니다 (`components.json` 설정 기반).
